...
def welcome(m):
    ...
    a = types.InlineKeyboardButton("Taylor Team \xE2\x9C\x8C", url="https://telegram.me/mmf_channel")
    c = types.InlineKeyboardButton("Add group \xE2\x9C\x8C", url="https://telegram.me/ID_mmf_robot?startgroup=test")
    markup.add(a, c)
    b = types.InlineKeyboardButton("Developer ID bot \xE2\x9C\x8C", url="https://telegram.me/Ww_king_mmf_wW")
    markup.add(b)
    nn = types.InlineKeyboardButton("Inline Mode", switch_inline_query='')
    markup.add(nn)
    ret_msg = bot.send_message(cid, "Hello I'm ID mmf bot \n\n Send : \n  /id or /me or /info   \n\n\n get your id : \n /idme (just pv) \nsend Your feedback : /feedback [msg]\n\n\n list inline mod : \ntype @ID_mmf_robot\n\nBot version 3", disable_notification=True, reply_markup=markup)
...
def id(m):      # info menu
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("\xF0\x9F\x98\x8A mmf_channel \xF0\x9F\x98\x8A", url="https://telegram.me/mmf_channel"))
#info text
...
def c(m):
    ...
    bot.send_contact(uid, phone_number="+98 910 294 4992", first_name="mmf")
...
def p(m):
    ...
    a = types.InlineKeyboardButton('mmf', url='https://telegram.me/Ww_king_mmf_wW')
    y = types.InlineKeyboardButton('\xF0\x9F\x92\x8E End \xF0\x9F\x92\x8E', callback_data='Team')
    ch = types.InlineKeyboardButton('Channel', url='https://telegram.me/mmf_channel')
    git = types.InlineKeyboardButton('Github', url='https://github.com/mohammadmahdi001/id_bot')
    markup.add(v)
    markup.add(a, y)
    markup.add(y)
    markup.add(ch, git)
    bot.send_chat_action(uid, 'typing')
    bot.send_message(uid, "mmf developer Telegram bot and web mastering \n\n developers : \n [mmf](https://telegram.me/mmf_channel)", parse_mode="Markdown")
    bot.send_photo(uid, open('mmf.jpg'), caption="@mmf_channel", reply_markup=markup)

@bot.message_handler(commands=['idbot'])
def handler(m):
    cid = m.chat.id
    bot.send_message(cid, "My Name is ID bot \n creator and developer : [mmf](https://telegram.me/mmf_channel) \n development channel : [Taylor Team](https://telegram.me/mmf_channel)\n\n [github](https://github.com/mohammadmahdi001/id_bot)", parse_mode="Markdown")
    bot.send_chat_action(cid, "upload_photo")
    bot.send_photo(cid, open('slackbot-story1-582x436.jpg'), caption="@ID_mmf_robot  \xF0\x9F\x98\x9C")
...
def query_text(query):
    ...
    #pic = types.InlineQueryResultPhoto('2',
                                       #'http://s3.img7.ir/RfApB.jpg',
                                       #'http://s3.img7.ir/QifrY.jpg',
                                       #input_message_content=types.InputTextMessageContent('@mmf_channel')
